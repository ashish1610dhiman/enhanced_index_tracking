...
def performance(price, result, C, T):
    ...
    in_excess_return = y_in.sum(axis=1).mean() - (in_returns["index"] * C).mean()
    out_excess_return = y_out.sum(axis=1).mean() - (out_returns["index"] * C).mean()
...
def EIT_bucket(kernel,bucket,bucket_no,failure,z_low,C,T,file,lamda,nuh,xii,k,pho,f,output, \
    ...
    return (status, LP.objective_value, selected_bucket, LP, in_excess_return,\
