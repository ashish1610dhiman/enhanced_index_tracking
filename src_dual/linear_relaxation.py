...
#         benchmark_return=returns["index"][t]*C/T
#         z.append((sum(portfolio_return)-benchmark_return)/benchmark_return)
#     return (sum(z))
...
# Solve LP Relaxation
LP = Model("Linear Relaxation of EIT-Dual")
...
result.to_csv(output + "/result_eit_dual_index_{}.csv".format(file), index=False)
# Initialisation
index = [1]
tracking = [1]
portfolio_return = []
...
plt.title("Linear Relaxation of EIT_dual for index={}\n".format(file))
plt.suptitle("\nxii={},k={},lambda={},nuh={}".format(xii, k, lamda, nuh), fontsize=11)
plt.savefig(output + "/LP_EIT_dual for index_{}.jpg".format(file), dpi=250)
