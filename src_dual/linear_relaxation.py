...
if len(sys.argv)!=6:
    ...
    file=int(sys.argv[1])
    T=int(sys.argv[2])
    w_return=float(sys.argv[3]) #weight for excess_return in objective
    w_risk=float(sys.argv[4]) ##weight for Tr-Error in objective
    k=int(sys.argv[5]) #Cardinality constraint
...
#Solve LP Relaxation
LP = mip.Model("Linear Relaxation of Dual-EIT",mip.MAXIMIZE)
...
""" Objective """
(excess_return,benchmark_return)=excess_return(returns,price,X_1,C)
LP+=w_return*(excess_return/benchmark_return)-w_risk*(mip.xsum([d[t]+u[t] for t in range(1,T+1)])/C)
...
status=LP.optimize()
print("***************************************************\n")
print("Optimisation Status={}".format(str(status)))
...
file1 = open("EIT_Dual_LP_details.txt","w+")
file1.writelines("LP(EIT_Dual) status={}\nObjective={}\n".format(str(status),str(round(LP.objective_value,3))))
file1.writelines("w_return={},w_risk={},k={},lambda={},nuh={}".format(w_return,w_risk,k,lamda,nuh))
file1.close()

#Write LP model to a file
LP.write("LP_EIT_Dual_index_{}.lp".format(file))
...
plt.title("Linear Relaxation of EIT Dual for index={}\n".format(file))
plt.suptitle("\nw_return={},w_risk={},k={},lambda={},nuh={}".format(w_return,w_risk,k,lamda,nuh),fontsize=11)
plt.savefig("LP_EIT_Dual for index_{}.jpg".format(file),dpi=250)