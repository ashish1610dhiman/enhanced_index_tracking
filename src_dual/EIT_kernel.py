...
def EIT_kernel(kernel,C,T,file,lamda,nuh,w_return,w_risk,k):
    import pandas as pd
    import mip.model as mip
    import numpy as np
    import random
    ...
    """ Define the EIT_Dual and necessary problem variables """
    #Solve EIT Kernel
    LP = mip.Model("EIT_Dual_kernel",mip.MAXIMIZE)
    ...
    result.to_csv("EIT_Dual_kernel_result_index_{}.csv".format(file),index=False)
    LP.write("EIT_Dual_kernel_for_index_{}.lp".format(file))
...
def plot_results(kernel,result,file,T):
    ...
    plt.title("EIT_Dual(kernel) performance for index={}\n".format(file))
    plt.savefig("EIT_Dual_kernel_for_index_{}.jpg".format(file),dpi=250)