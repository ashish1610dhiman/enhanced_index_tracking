...
def EIT_kernel(kernel, C, T, file, lamda, nuh, xii, k, pho, f, output, w_return, w_risk, w_risk_down, from_root=True):
    if from_root:
...
def plot_results(kernel, result, file, T, output, from_root=True):
    ...
    plt.savefig(output + "/EIT_dual_kernel_for_index_{}.jpg".format(file), dpi=250)
    plt.close()
