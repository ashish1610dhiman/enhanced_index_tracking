...
def EIT_bucket(kernel,bucket,bucket_no,failure,z_low,C,T,file,lamda,nuh,xii,k):
    import pandas as pd
    import mip.model as mip
    import numpy as np
    import random
    ...
    if status==0:
        ...
        result.to_csv("EIT_bucket_{}_result_index_{}.csv".format(bucket_no,file),index=False)
    print("***************************************************")
    LP.write("EIT_bucket_{}_for_index_{}.lp".format(bucket_no,file))
